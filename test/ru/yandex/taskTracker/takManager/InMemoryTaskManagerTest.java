...
public class InMemoryTaskManagerTest extends TaskManagerTest<InMemoryTaskManager> {
    ...
    void shouldSetStatusNewWhenAllSubtasksNew() {
        ...
        taskManager.checkStatusOfEpic(epic2);
        assertEquals(Status.NEW, epic2.getStatus(), "Неверный статус");
    }
    ...
    void shouldSetStatusDoneWhenAllSubtasksDone() {
        ...
        taskManager.addSubtask(new Subtask("1", "2", Status.DONE, 1, TaskType.SUBTASK,
                5, LocalDateTime.of(2023, 1, 1, 0, 30)));
        taskManager.checkStatusOfEpic(epic2);
        assertEquals(Status.DONE, epic2.getStatus(), "Неверный статус");
    }
    ...
    void shouldSetStatusInProgressWhenAllSubtasksInProgress() {
        ...
        taskManager.addSubtask(new Subtask("1", "2", Status.IN_PROGRESS, 1, TaskType.SUBTASK,
                5, LocalDateTime.of(2023, 1, 1, 0, 30)));
        taskManager.checkStatusOfEpic(epic2);
        assertEquals(Status.IN_PROGRESS, epic2.getStatus(), "Неверный статус");
    }
    ...
    void shouldSetTimeMaxWhenNoSubtasks() {
        taskManager.checkTimeOfEpic(epic2);
        assertNotNull(epic2.getStartTime(), "Время не установлено");
        assertEquals(LocalDateTime.MAX, epic2.getStartTime(), "Время установлено неверно");
    }
    ...
    void shouldSetTimeOfSubtask() {
        ...
        taskManager.checkTimeOfEpic(epic2);
        assertNotNull(epic2.getStartTime(), "Время не установлено");
        assertEquals(subtask3.getStartTime(), epic2.getStartTime(), "Время установлено неверно");
    }
}
